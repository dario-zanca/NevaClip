...
class NeVAWrapper(nn.Module):
    ...
    def forward(self, x, foveation_positions):
        ...
        foveation_area = get_foveation(self.foveation_aggregation, self.foveation_sigma, self.image_size, foveation_positions, self.device)
    ...
    def initialize_scanpath_generation(self, x, batch_size):
        ...
        self.blur = calculate_blur(x, self.blur_filter_size, self.blur_sigma, self.device)
    ...
    def run_optimization(self,
        ...
        with torch.cuda.amp.autocast():
            ...
            for step in range(scanpath_length):
                ...
                if len(output2) > 0:
                    l = (step / scanpath_length) ** 2
                    output = l * labels + (1 - l) * output2
                else:
                    output = labels

                targets = self.target_function(x, output)

                foveation_pos = torch.zeros((batch_size, 2, 1, 1), device=self.device, requires_grad=True)
                best_foveation_pos = torch.zeros((batch_size, 2, 1, 1), device=self.device)  # * 2 - 1
                ...
                for _ in range(opt_iterations):
                    ...
                    # save best
                    idxs = loss < best_loss
                    best_loss[idxs] = loss[idxs].float()
                ...
                current_foveation_mask = get_foveation(self.foveation_aggregation, self.foveation_sigma,
                                                       self.image_size, best_foveation_pos, self.device)
        ...
        return torch.stack(scanpath, 1).squeeze()


def calc_gaussian(a, std_dev, image_size, positions, device):
    B = positions.shape[0]
    xa, ya = create_grid(B, image_size, device)
    xa = xa - positions[:, 0]
    ya = ya - positions[:, 1]
    distance = (xa**2 + ya**2)
...
def create_grid(batch_size, size, device="cuda"):
...
def calculate_blur(images, blur_filter_size, sigma=5, device="cuda"):
...
def get_foveation(foveation_aggregation, foveation_sigma, image_size, positions, device):
    mask = calc_gaussian(foveation_aggregation, foveation_sigma, image_size, positions, device)
    return mask
